...
class CCAvenue(object):
    ...
    def handle_start(self, request):
        ...
        merchant_data = '&'.join([
            ...
            'currency={}'.format(payment.amount.default_currency.code),
            ...
            'billing_name={}'.format(self._billing['NAME']),
            'billing_address={}'.format(self._billing['ADDRESS']),
            'billing_city={}'.format(self._billing['CITY']),
            'billing_state={}'.format(self._billing['STATE']),
            'billing_zip={}'.format(self._billing['ZIP']),
            'billing_country={}'.format(self._billing['COUNTRY']),
            'billing_tel={}'.format(self._billing['TEL']),
            'billing_email={}'.format(self._billing['EMAIL']),
            'delivery_name={}'.format(self._billing['NAME']),
            'delivery_address={}'.format(self._billing['ADDRESS']),
            'delivery_city={}'.format(self._billing['CITY']),
            'delivery_state={}'.format(self._billing['STATE']),
            'delivery_zip={}'.format(self._billing['ZIP']),
            'delivery_country={}'.format(self._billing['COUNTRY']),
            'delivery_tel={}'.format(self._billing['TEL']),
            'delivery_email={}'.format(self._billing['EMAIL']),
            'merchant_param1=',
            'merchant_param2=',
            'merchant_param3=',
            'merchant_param4=',
        ...
        return render('finance/ccavenue/start.html', context={
    ...
    def handle_cancel(self, request):
        ...
        return render('finance/ccavenue/cancel.html', context={'response': resp})

    def handle_success(self, request):
        enc_resp = request.POST['encResp']
        resp = decrypt(enc_resp, self._working_key)
        return render('finance/ccavenue/success.html', context={'response': resp})
